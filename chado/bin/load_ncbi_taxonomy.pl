...
if ( !$dbhost && !$dbname ) {
	die "Need -D dbname and -H hostname arguments.\n";
}
...
my $db =
  $schema->resultset("General::Db")->find_or_create( { name => $db_name } );
...
$schema->resultset('Phylogeny::Phylonode')
  ->search( { phylotree_id => $phylotree_id } )->delete();
...
while ( my $line = <NODE> ) {
	...
	if ( !( $okay_level{$level} ) ) {
		message("Node rank not found in the database ("
			  . $okay_level{$level}
			  . "). Skipping\n" );
		next;
	}
	...
	if ($infile) {
		if ( exists $tax_file{$id} ) {
			...
			if ( $parent && !( exists $tax_file{$parent} ) ) {
				message(
					"Parent $parent for tax_id '"
					  . $id
					  . "' does not exist in your input file ! This means $id is your root, or you need to check your input!\n",
					1
				);
			}
		}
...
foreach my $id ( keys %node ) {
	...
	if ( !$tax_file{$parent_taxid} ) {
		message(
			"No parent id found for  species "
			  . $node{$id}{'name'}
			  . " (id = '"
			  . $id
			  . "') !! This means your species is the root node, or there is an error in yout input file \n\n",
			1
		);
	}
...
"NO GENUS OR SPECIES FOUND FOR tax_id $genbank_taxon_accession! Check your input file! \n";
		...
		my $taxonomy_cv =
		  $schema->resultset("Cv::Cv")->find( { name => 'taxonomy' } );
		if ( !$taxonomy_cv ) {
			die
"No cv found for 'taxonomy' . Did you run 'load_taxonomy_cvterms.pl' ? See the perldoc for more details \n\n";
		}
		my ($level_cvterm) =
		  $taxonomy_cv->find_related( "cvterms", { name => $level } );
		...
		if ( !$level_cvterm ) {
			...
			print ERR
"No cvterm found for type $level! Check your cvterm table for loaded taxonomy (cv name should be 'taxonomy') \n\n";
			die
"No cvterm found for type $level! Check your cvterm table for loaded taxonomy (cv name should be 'taxonomy') . See perldoc load_taxonomy_cvterms.pl \n\n";
		}
...
"An error occured! Rolling back! \n $_ \n Resetting database sequences...\n",
	...
	foreach my $key ( keys %seq ) {
		...
		if ($maxvalue) {
			$dbh->do("SELECT setval ('$value', $maxvalue, true)");
		}
